...
def main():
    ...
    parser.add_option('--max_seq_length', type=int, default=400,
    ...
    parser.add_option('--basedir', type=str, default='data/congress/relevance/basic/',
    ...
    parser.add_option('--output-prefix', type=str, default=None,
                      help='Output directory prefix: default=(AUTO)')
...
def run_folds(basedir, seed=42, model_type='bert', model_name_or_path='bert-base-uncased', tokenizer_name='', folds=5, start_fold=0, lr=2e-5, max_seq_length=400, per_gpu=4, n_epochs=7, train_file='train.jsonlist', output_prefix=None):
    ...
    for fold in range(start_fold, folds):
        ...
        cmd = ['python', '-m', 'hf.run',
               ...
               '--weight_field', 'weight',
               '--metrics', 'accuracy,f1,weighted_accuracy,weighted_f1',
               '--seed', str(seed)
               ]
        if model_type == 'bert':
        ...
        with open(os.path.join(folds_dir, 'my_config.json'), 'w') as f:
